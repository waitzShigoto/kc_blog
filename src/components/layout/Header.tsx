          ...
          <div className="flex items-center space-x-4">
              ...
              {isMobileMenuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-background border border-border rounded-lg shadow-lg py-2 z-50">
                  ...
                  >
                    ...
                    <span>{locale === 'zh' ? '人工智慧' : locale === 'en' ? 'AI' : '人工知能'}</span>
                  </Link>
            ...
            <div className="hidden sm:flex items-center space-x-3">
              ...
              <Link
                href={`/${locale}/ai`}
                className="p-2 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-200 flex items-center gap-1"
                title={locale === 'zh' ? '人工智慧' : locale === 'en' ? 'AI' : '人工知能'}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                </svg>
                <span className="text-xs">{locale === 'zh' ? '人工智慧' : locale === 'en' ? 'AI' : '人工知能'}</span>
              </Link>
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 rounded-md text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200"
                  title={locale === 'zh' ? '加入我的最愛' : locale === 'en' ? 'Add to Favorites' : 'お気に入りに追加'}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {showBookmarkTip && (
                  <div className="absolute top-full right-0 mt-2 z-[100] animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="bg-card text-card-foreground border border-border px-4 py-3 rounded-xl shadow-2xl text-sm whitespace-nowrap flex flex-col items-center gap-1 min-w-[200px]">
                      <div className="flex items-center gap-2 font-medium">
                        <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
                        {locale === 'zh' ? '小撇步' : locale === 'en' ? 'Pro Tip' : 'ヒント'}
                      </div>
                      <div className="text-muted-foreground">
                        {locale === 'zh'
                          ? `請按下 ${navigator.platform.toUpperCase().indexOf('MAC') >= 0 ? 'Cmd+D' : 'Ctrl+D'} 收藏頁面`
                          : locale === 'en'
                            ? `Press ${navigator.platform.toUpperCase().indexOf('MAC') >= 0 ? 'Cmd+D' : 'Ctrl+D'} to Bookmark`
                            : `${navigator.platform.toUpperCase().indexOf('MAC') >= 0 ? 'Cmd+D' : 'Ctrl+D'} でお気に入り登録`}
                      </div>
                      {/* 下箭頭裝飾 */}
